import 'package:flutter/material.dart';

class AppColors {
  // Основные цвета градиентов
  static const Color primaryGradientStart = Color(0xFF667EEA);
  static const Color primaryGradientMiddle = Color(0xFF764BA2);
  static const Color primaryGradientEnd = Color(0xFFF093FB);

  // Элегантные цвета для desktop
  static const Color desktopPrimary = Color(0xFF2563EB);
  static const Color desktopSecondary = Color(0xFF7C3AED);
  static const Color desktopAccent = Color(0xFF06B6D4);
  static const Color desktopSuccess = Color(0xFF10B981);
  static const Color desktopWarning = Color(0xFFF59E0B);
  static const Color desktopError = Color(0xFFEF4444);

  // Light Theme Colors
  static const Color lightPrimary = Color(0xFF6366F1);
  static const Color lightPrimaryVariant = Color(0xFF4F46E5);
  static const Color lightSecondary = Color(0xFF10B981);
  static const Color lightBackground = Color(0xFFF8FAFC);
  static const Color lightSurface = Color(0xFFFFFFFF);
  static const Color lightSurfaceVariant = Color(0xFF1E293B); // Темный sidebar на светлом фоне
  static const Color lightError = Color(0xFFEF4444);
  static const Color lightOnPrimary = Color(0xFFFFFFFF);
  static const Color lightOnSecondary = Color(0xFFFFFFFF);
  static const Color lightOnBackground = Color(0xFF1E293B);
  static const Color lightOnSurface = Color(0xFFFFFFFF); // Белый текст на темном sidebar
  static const Color lightOnSurfaceVariant = Color(0xFFFFFFFF); // Белый для иконок на темном sidebar
  static const Color lightOnError = Color(0xFFFFFFFF);
  static const Color lightOutline = Color(0xFFE2E8F0);
  static const Color lightOutlineVariant = Color(0xFFF1F5F9);
  static const Color lightShadow = Color(0x1A000000);
  
  // Dark Theme Colors
  static const Color darkPrimary = Color(0xFF818CF8);
  static const Color darkPrimaryVariant = Color(0xFF6366F1);
  static const Color darkSecondary = Color(0xFF34D399);
  static const Color darkBackground = Color(0xFF0F172A);
  static const Color darkSurface = Color(0xFF1E293B);
  static const Color darkSurfaceVariant = Color(0xFFFFFFFF); // Светлый sidebar на темном фоне
  static const Color darkError = Color(0xFFF87171);
  static const Color darkOnPrimary = Color(0xFF0F172A);
  static const Color darkOnSecondary = Color(0xFF0F172A);
  static const Color darkOnBackground = Color(0xFFF8FAFC);
  static const Color darkOnSurface = Color(0xFF1E293B); // Темный текст на светлом sidebar
  static const Color darkOnSurfaceVariant = Color(0xFF1E293B); // Темный для иконок на светлом sidebar
  static const Color darkOnError = Color(0xFF0F172A);
  static const Color darkOutline = Color(0xFF334155);
  static const Color darkOutlineVariant = Color(0xFF475569);
  static const Color darkShadow = Color(0x40000000);
  
  // Цвета для ошибок
  static const Color errorColor = Color(0xFFEF4444);
  static const Color errorColorLight = Color(0xFFFEE2E2);
  
  // Gradient Colors
  static const List<Color> primaryGradient = [
    Color(0xFF6366F1),
    Color(0xFF8B5CF6),
  ];
  
  static const List<Color> secondaryGradient = [
    Color(0xFF10B981),
    Color(0xFF06B6D4),
  ];
  
  static const List<Color> darkGradient = [
    Color(0xFF1E293B),
    Color(0xFF334155),
  ];
  
  // Social Colors
  static const Color googleRed = Color(0xFFDB4437);
  static const Color appleBlack = Color(0xFF000000);
  static const Color facebookBlue = Color(0xFF1877F2);
  
  // Status Colors
  static const Color success = Color(0xFF10B981);
  static const Color warning = Color(0xFFF59E0B);
  static const Color info = Color(0xFF3B82F6);
  
  // Navigation Colors
  static const Color navigationPrimary = Color(0xFF6366F1);
  static const Color navigationPrimaryVariant = Color(0xFF4F46E5);
} 